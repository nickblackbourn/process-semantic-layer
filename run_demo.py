"""
Process Semantic Layer - Demo

Run this to see the system in action with visual results.

Usage:
    python run_demo.py              # Automated demo with performance metrics
    python run_demo.py --interactive # Interactive query mode
"""
import sys
import argparse
...
def draw_ranking_visual(results, query_concepts):
    """Draw a visual ranking chart with percentage bars."""
    if not results:
    ...
    for i, result in enumerate(results, 1):
        # Calculate bar length based on actual score (0-100%)
        # Max bar width is 40 chars for scores 0.0-1.0
...
def show_concept_matching(matched_concepts, total_docs, filtered_docs):
    ...
    if matched_concepts:
        ...
        print(f"  ✓ Filtering enabled: {filtered_docs}/{total_docs} documents")
    else:
        print(f"\n  🔍 NO CONCEPT MATCH - Full corpus search ({total_docs} documents)")


def print_result(rank, result):
    """Print a formatted search result (for interactive mode)."""
    print(f"\n{rank}. {result.title}")
...
def run_automated_demo(pipeline):
    """Run automated demo with visual metrics."""
    concept_graph = pipeline.concept_graph
...
def run_interactive_mode(pipeline):
    ...
    print("="*70)
    print("\n💡 Enter queries to search the document collection.")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Process Semantic Layer Demo",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python run_demo.py              # Run automated demo with metrics
  python run_demo.py --interactive # Interactive query mode
        """
    )
    parser.add_argument(
        '--interactive', '-i',
        action='store_true',
        help='Run in interactive mode (enter your own queries)'
    )
    ...
    # Initialize system
    print("\n" + "█"*70)
    print("█" + " "*15 + "PROCESS SEMANTIC LAYER DEMO" + " "*27 + "█")
    print("█"*70)
    print("\n🚀 Initializing system...")
    ...
    print(f"\n✓ Loaded {len(concept_graph.concepts)} concepts")
